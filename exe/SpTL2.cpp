...
<kernel (-1: SpTL2 , -2: SpTL2so) (optional)> \
...
//	printf("here\n")
	...
	*/
	...
	int last_mode_id = t->modeid[nmode -1];
...
//	printf("Total MTTKRP time %lf \n",total);
	...
	// execute for the last mode
	{
		mode = 0;
		auto start = std::chrono::high_resolution_clock::now();

		mttkrp_hardwired_first_not_fused(t,mode,r,mats,profile);
		//printf("here %lf\n",mats[mode]->val[0] );
		//printf("here\n");
		auto end = std::chrono::high_resolution_clock::now();
		std::chrono::duration<double> diff = end-start;
